...
Includes file transfer via the [+] attach button and incoming file display.

Received files are auto-saved to a 'received/' folder next to this script.
The chat shows the filename, size, and saved path as a clickable-style notice.
"""
...
def _save_received_file(filename, data_bytes):
    """Save bytes to received/ folder, avoiding overwrites. Returns saved path."""
    os.makedirs(RECEIVED_DIR, exist_ok=True)
    base, ext = os.path.splitext(filename)
    dest = os.path.join(RECEIVED_DIR, filename)
    counter = 1
    while os.path.exists(dest):
        dest = os.path.join(RECEIVED_DIR, f"{base}_{counter}{ext}")
        counter += 1
...
class ChatScreen(tk.Frame):
    ...
    def _build_chat_panel(self, parent):
        ...
        self.display.tag_config("file_notice",
            foreground=AMBER, font=("Courier New", 9))
        ...
        # [+] button now opens file picker and sends the file
    ...
    def _add_message(self, sender, body, is_file=False):
        if not self.current_chat:
            return
        ts    = datetime.datetime.now().strftime("%H:%M")
        entry = {"from": sender, "body": body, "ts": ts, "is_file": is_file}
    ...
    def append_incoming(self, chat_name, sender, body, ts=""):
        """Called via after() for incoming text messages."""
    ...
    def append_incoming_file(self, chat_name, sender, filename, data_bytes, ts=""):
        """
        Called via after() when a FILE_SEND arrives.
        Auto-saves to received/ and shows a notice in the chat.
        """
        try:
            dest     = _save_received_file(filename, data_bytes)
            size_kb  = len(data_bytes) / 1024
            notice   = f"[FILE] {filename}  ({size_kb:.1f} KB)  -> {dest}"
        except Exception as e:
            notice = f"[FILE] {filename} — could not save: {e}"
    ...
    def _attach(self):
        """Open file picker and send the chosen file to the current chat."""
        if not self.current_chat:
            messagebox.showinfo("No chat", "Select a user or group first.")
            return
        ...
        if file_size > 5 * 1024 * 1024:
            ...
            if not messagebox.askyesno("Large file",
                                       f"'{filename}' is {size_mb:.1f} MB. Send?"):
                return

        if not self.net:
            self._add_message("system", f"[Preview mode] Would send: {filename}")
            return

        # Show sending notice immediately
        self._add_message("system", f"Sending '{filename}'...")

        # Send in a background thread so the GUI doesn't freeze on large files
        import threading
        def do_send():
            if self.current_type == "group":
                self.net.send_file_to_group(self.current_chat, path)
            else:
                self.net.send_file(self.current_chat, path)
            size_kb = file_size / 1024
            self.after(0, self._add_message, self.username,
                       f"[FILE] {filename}  ({size_kb:.1f} KB)  sent")
...
class App(tk.Tk):
    ...
    def _do_login(self, username, password, host, port):
        ...
        def on_file_received(sender, filename, data_bytes, ts, chat_name):
            chat.after(0, chat.append_incoming_file,
                       chat_name, sender, filename, data_bytes, ts)
        ...
        net = NetworkClient(
            ...
            error=on_error,
            users=on_users,
            file_received=on_file_received
        )
